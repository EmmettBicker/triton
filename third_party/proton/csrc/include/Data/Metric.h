...
public:
  FlexibleMetric(const std::string &valueName,
    ...
    exclusive = contain(toLower(valueName), "(exc)");
    property = contain(toLower(valueName), "(pty)");
...
private:
  ...
  const static inline std::string VALUE_NAMES[kernelMetricKind::Count] = {
      ...
      "time (ns)",       "device_id (exc)", "device_type (exc)",
  };
};
...
public:
  ...
  virtual bool isProperty(int valueId) const { return true; }
